...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy { c => c.toLower.charValue }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      (for {
        len <- (1 to count).toList
        newCombinations <- tailCombinations map { occurrence => (char, len) +: occurrence }
      } yield newCombinations) ::: tailCombinations
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y foldLeft SortedMap[Char, Int]() ++ x) { case (map, (charVal, charCount)) =>
      val newCount = map(charVal) - charCount
      if (newCount != 0) map updated (charVal, newCount)
      else map - charVal
    }.toList
  }
    ...
    def iter(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List())
        ...
        for {
          ...
          anagramList <- dictionaryByOccurrences.getOrElse(combination, List())
          iterList <- iter(subtract(occ, combination))
        } yield anagramList :: iterList
    }
